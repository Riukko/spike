...
#endif
{
	...
	public void OnPreprocessBuildInternal(UnityEditor.BuildTarget target, string path)
	{
		...
		if (AkWwiseEditorSettings.Instance.CopySoundBanksAsPreBuildStep)
		{
			...
			{
				if (string.IsNullOrEmpty(AkWwiseEditorSettings.Instance.WwiseInstallationPath))
					UnityEngine.Debug.LogFormat("WwiseUnity: SoundBank folder has not been copied for <{0}> target at <{1}>. This will likely result in a build without sound!!!", target, path);
				else
